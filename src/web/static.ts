...
const PAGE_FILES: Readonly<Record<PageName, string>> = {
  ...
  hire: 'hire.html',
  // P8h: same stance as hire above -- /agreement owns no path an API
  // route also serves.
  agreement: 'agreement.html',
  // P8i: same stance -- /deposit owns no path an API route also serves.
  deposit: 'deposit.html',
  // P8j: same stance -- /staged owns no path an API route also serves.
  staged: 'staged.html',
  // P8l: same stance -- /pullrequest owns no path an API route also
  // serves.
  pullrequest: 'pullrequest.html',
  // P8m: same stance -- /myjobs owns no path an API route also serves.
  myjobs: 'myjobs.html',
  // P8n: same stance -- /myagents owns no path an API route also serves.
  ...
  // (the API route is /buyers/:githubLogin/conduct, a different path).
  ...
  // HT1 Part B: same stance -- /notifications owns no path an API route
  // also serves (the API routes are GET /accounts/:did/notifications
  // and its siblings, a different path shape entirely).
...
export function createWebSurface(
  ...
  return {
    mountPages(app: Express): void {
      ...
      // P8h: same stance as /hire above -- /agreement owns no path an API
      // route also serves.
      app.get('/agreement', (_req: Request, res: Response) => send(res, 'agreement'));
      // P8i: same stance -- /deposit owns no path an API route also
      // serves.
      app.get('/deposit', (_req: Request, res: Response) => send(res, 'deposit'));
      // P8j: same stance -- /staged owns no path an API route also serves.
      app.get('/staged', (_req: Request, res: Response) => send(res, 'staged'));
      // P8l: same stance -- /pullrequest owns no path an API route also
      // serves.
      app.get('/pullrequest', (_req: Request, res: Response) => send(res, 'pullrequest'));
      // P8m: same stance -- /myjobs owns no path an API route also serves.
      app.get('/myjobs', (_req: Request, res: Response) => send(res, 'myjobs'));
      // P8n: same stance -- /myagents owns no path an API route also
      // serves.
      app.get('/myagents', (_req: Request, res: Response) => send(res, 'myagents'));
      ...
      // P8s: same stance -- /conduct owns no path an API route also
      // serves (the API route is /buyers/:githubLogin/conduct, a
      // different path, so this is a plain own-path mount, never
      // `negotiated`).
      app.get('/conduct', (_req: Request, res: Response) => send(res, 'conduct'));
      ...
      // HT1 Part B: same stance -- /notifications owns no path an API
      // route also serves, so this is a plain own-path mount, never
      // `negotiated`.
      app.get('/notifications', (_req: Request, res: Response) => send(res, 'notifications'));
