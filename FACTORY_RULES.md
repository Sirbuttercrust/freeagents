...
<!--
  ...
  Filled in 2026-08-17. Every value here is factory
  MECHANICS (caps, gates, protected paths) — the product scope lives in
  MISSION.md and is the operator's to write.
-->
...
8. **Never modify the holdout, the mutation set, or the ratchet floor** —
   `.factory/holdout/**`, `harness/mutations/**`, `harness/locks/**` — or their
   enforcement path (the holdout guard hook, the `forge`
   profile's `hooks:` block). A builder that can reach the assertions it is
   judged against can satisfy them for the wrong reasons, and every gate
   downstream assumes it cannot.
...
1. Static checks pass - `python harness/ci.py --quick`
2. Unit and integration tests pass - `python harness/ci.py`
...
4. **[CODE]** The end-to-end path ran and passed. `E2E_PASSED` appears, with a step
   count at or above the floor in `.factory/locks/floor.json`
   (`e2e_steps_asserted`), which only a human commit may raise.
...
10. Fix attempts ≤ 2 (`FACTORY_MAX_FIX_ATTEMPTS`).
...
3, driven by `harness/e2e.py` against a running instance.
...
**Infrastructure:** `Dockerfile*`, `docker-compose*.yml`, `deploy/**`, `infra/**`,
  `*.service`, `*.timer`, `factory/**`, `*.plist`, `*.ps1`
**Secrets and auth:** `.env*`, agent profile directories (they hold tokens),
  `secrets.*`, `credentials.*`, `*.local`
**Security invariants:** `.factory/holdout/**`, `harness/mutations/**`,
  `harness/locks/**`, the holdout guard hook
...
- a public or irreversible external side effect — a sent email, a published
  package, a posted message, anything leaving this machine
...
- Concurrency: 1 workflow at a time (one 24 GB GPU serves one model). Above one, a per-target lock is mandatory - never dispatch a workflow whose (workflow, target) pair is already in flight.
- Fix attempts per PR: 2
- PR size: 500 lines, 12 files (`FACTORY_SIZE_CAP`, `FACTORY_FILE_CAP`)
- **Dispatcher priority order:** fix a PR → validate a PR → implement an issue →
  triage. Finish in-flight work before starting new work.
- **Stop button:** two, because they fail differently. `.factory/STOP` (a file;
  works with the network down) and the `factory:stop` label (reachable from a
  phone; fails CLOSED). Killing inference: run `pc_llama_server.ps1 -Stop` on
  the PC — it terminates `llama-server` rather than idling it, because an idle
  server still pins ~18 GB of VRAM and the machine is the operator's for gaming.
