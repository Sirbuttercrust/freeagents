...
#
# PORTED FOR THIS REPO, 2026-08-19. The template shipped `["app", "tests",
# "harness", ".factory"]`, which is a Python layout: this project has no `app/`
# directory, so every mutation would have been applied to a copy missing the
# file it mutates and reported NOT_INJECTED. A mutation set that never injects
# is worse than none, because it prints a score.
#
# node_modules is DELIBERATELY NOT COPIED and instead symlinked below: copying
# it takes minutes per defect and there are seven of them.
COPY = ["src", "tests", "harness", "prisma", "package.json", "package-lock.json",
        "tsconfig.json", "vitest.config.ts", "eslint.config.js",
        "MISSION.md", "FACTORY_RULES.md", ".factory"]
SKIP_DIRS = {"__pycache__", ".git", "runs", "locks-runtime", "builds",
             ".worktrees", "node_modules", "dist"}
...
def build_copy(dest: Path) -> None:
    ...
    # node_modules is symlinked rather than copied: this repo's tree is ~200MB
    # and seven copies of it turns a two-minute run into twenty. The build never
    # writes to node_modules, so sharing it is safe.
    nm = ROOT / "node_modules"
    if nm.exists() and not (dest / "node_modules").exists():
...
def main() -> int:
    ...
    for d in defects:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            #
            # FACTORY_HOLDOUT_REPO is UNSET for the inner run on purpose. The
            # holdout asserts MISSION invariants against a repo root, and these
            # mutations are ordinary logic bugs, not invariant violations. Left
            # set, every inner gate would also run the holdout against the
            # throwaway copy: slower, and it would credit the holdout with
            # catching defects the product's own tests are supposed to catch.
            # What this file measures is whether THE GATE notices a broken
            # build, so the inner gate is the product's own checks.
            env = dict(os.environ, FACTORY_IN_MUTATION="1")
            env.pop("FACTORY_HOLDOUT_REPO", None)
