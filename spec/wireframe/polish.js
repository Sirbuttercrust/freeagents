...
(function () {
  ...
  /* ---------------------------------------------------------------- sheets
     The payment screens' modal sheets, folded in from flow.js on 2026-09-09
     during the D1 reconcile. They came across because dropping flow.js
     without them would have silently broken seven sheets: the deposit scan,
     the balance payment, the redo picker, the decline confirm, the cited
     close, and the operator's accept and refuse. A screen whose primary
     action does nothing is worse than one that was never restyled.

     NATIVE <dialog>, DELIBERATELY. showModal() gives focus containment,
     Escape, the inert background and the ::backdrop for free. A hand-rolled
     overlay reimplements four things and gets at least one of them subtly
     wrong, and the one it gets wrong is usually focus, which is invisible
     until somebody tabs. */
  function sheets() {
    /* Anything with data-opens="id" opens that dialog. showModal is
       feature-detected rather than assumed: where it is missing the dialog
       opens non-modally, which is a worse experience and not a broken one. */
    ...
    /* Clicking the backdrop closes. The target of a click on the backdrop is
       ...
       identity test is the whole check. */
