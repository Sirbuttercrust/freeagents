...
const ALLOWED_ABSENT: Record<string, Record<string, string>> = {
  ...
  deposit: {
    ...
    // "Open this in your wallet" (src/web/pages/deposit.html, the h2#scanh).
    // S1 dropped "DID" from that heading: DID is on the hire journey's
    // no-jargon list, and the one wallet that opens the address needs no
    // naming in the one place it is asked for.
    ...
    'Scan with your DID Wallet': 'the wireframe\u2019s own comment (deposit.html:238-240) says its drawn QR "encodes nothing"; the built sheet ships a real mechanism instead, a one-time payment URL in a readonly input with a Copy button headed "Open this in your wallet" (src/web/pages/deposit.html, the h2#scanh), because a heading promising a scan with nothing to scan is the inert-declared-control defect in copy',
  },
  credential: {
    // The wireframe draws the closed-without-shipping state as a second
