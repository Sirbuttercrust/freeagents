...
describe('agent delegation, invariant 2 (R-2): W3C verifiability', () => {
  ...
  it('read-back serves the same credential, and it still verifies', async () => {
    ...
    expect(body).toEqual({
      ...
      // AV1: the resolved avatar spec joined the contract at the same
      // time, unconditionally -- an agent with no override still renders
      // the DID-derived default (ENT-2.3), so there is no state to wait
      // on and no key that ever goes missing.
